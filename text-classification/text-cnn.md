...
* inputs: 使用了word embedding  ,不同于图像的像素，大部分NLP任务的输入是句子或文档作为一个矩阵。矩阵的每一行表示一个token，一般是一个分词，但也可以是一个字，总之每行都是一个向量，代表一个word
...
* 会使用多个filter window size ，每个window\_size又有num\_filters个卷积核。 
